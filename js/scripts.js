...
$('#calculate').click(function () {
    if (($('#cPrice').val().replace(/[^0-9.]/g, "")) == '') {
        ...
        $('input[name="u-price"]').val(numberWithSpaces(cPrice) + ' ' + $('#curr').val());
...
function calculate() {
    var cYear = $('#cYear').val(),
        ...
        jpan = Number($('#jpan').text().replace(/[^0-9.]/g, "")),
        frat = Number($('#frat').text().replace(/[^0-9.]/g, "")),
    ...
    var finishPrice = cPrice + ourComm + (jpan * USDRUB) + (frat * USDRUB) + brocker + epts + sbor + customs + recyclingCoff + nds;
    totalRub.text(numberWithSpaces(finishPrice.toFixed(1)) + ' руб');

    // Наша комисия +20.000 за 500.000------------------------------------
    finishPrice >= 500000 ? ourComm = ourComm + ((Math.ceil(finishPrice / 500000) - 1) * 20000) : ourComm = 60000;
    ...
    totalUsd.text(numberWithSpaces(totUsd.toFixed(1) + " $"));

    // присваиваем в таблицу ---------------------------
    priceLabelUsd.text((cPrice / USDRUB).toFixed(1) + ' $');
    priceLabel.text(numberWithSpaces(cPrice.toFixed(1)) + ' руб');
    $('#sbor').text(numberWithSpaces(sbor) + ' руб');

    recycling.text(numberWithSpaces(recyclingCoff.toFixed()) + ' руб');
    $("#nds").text(numberWithSpaces(nds.toFixed(1)) + ' руб');
    $("#customs").text(numberWithSpaces(customs.toFixed(1)) + ' руб');
    $('#fratRub').text(numberWithSpaces((frat * USDRUB).toFixed(1)) + " руб");
    $('#jpanRub').text(numberWithSpaces((jpan * USDRUB).toFixed(1)) + " руб");
    $('#ourCommUsd').text(numberWithSpaces((ourComm / USDRUB).toFixed(1)) + " $");
    $('#ourCommRub').text(numberWithSpaces(ourComm.toFixed(1)) + " руб");
